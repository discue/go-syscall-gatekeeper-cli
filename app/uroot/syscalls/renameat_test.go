...
func TestRenameAtAllowed(t *testing.T) {
	td := t.TempDir()
	...
	if !IsRenameAtAllowed(s, true) {
		t.Fatalf("expected renameat allowed for %s -> %s", old, newp)
	}
}

func TestRenameAtNotAllowed(t *testing.T) {
	td := t.TempDir()
	other := t.TempDir()
	runtime.Get().FileSystemAllowedPaths = []string{td}
	runtime.Get().FileSystemAllowWrite = true
	...
	if IsRenameAtAllowed(s, true) {
		t.Fatalf("expected renameat NOT allowed for %s -> %s", old, newp)
	}
}
